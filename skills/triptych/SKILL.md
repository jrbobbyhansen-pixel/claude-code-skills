...
- **One short video** (10–30s) walking the surface's core interaction — scroll, tap-through, the signature
  moment. Motion is half of UX; stills alone under-sell the concept whose bet is motion.
- Save to the design record: `<project>/design/triptych/<yyyy-mm-dd>-<target-slug>/` as
  `01-<thesis-slug>.png`, `01-<thesis-slug>-<state>.png`, `01-<thesis-slug>.mp4`, numbered in thesis order.
...
For each concept (in thesis order):
  <screenshot(s) displayed inline>  ·  video: <path>
  **<Name>** — <the bet, one sentence>
  Optimizes for: …   Costs you: …
  Axes: layout / hierarchy / density / motion / temperature / nav
Fit-to-brief: score each concept against the Phase-0 adjectives + the ONE job (be honest, not diplomatic).
Recommendation: at most one sentence, clearly labeled as yours.
Ask: "Pick one, name a hybrid (e.g. B's layout + C's motion), or say 'push further on <X>' for another round."
```
...
- Implement the pick for real — winner as-is, or the named hybrid — to shipping quality on the branch.
- **Delete** the losing variants and the switcher. No dead code, no "keeping B around just in case"; B lives
  on in the capture folder and git history.
