...
## Pre-flight (once)
1. Resolve scope/flags: whole app (default) · path · theme · `--target` · `--loops N` (default 3, **min 3**) · `--restart`.
...
3. Fresh run: `scripts/init.sh [base]` (gitignore `.ascend/` first · stash dirty tree · create `ascend/integration`;
   not-a-git-repo → stop, offer `git init`/snapshots, never auto-apply without revert) → `scripts/detect.py` (Project
   Profile) → `scripts/scan.py` (slices) → `scripts/state.py init --scope … --loops N --stack <type>`.
